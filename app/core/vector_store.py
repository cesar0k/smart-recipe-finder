...
class VectorStore:
    ...
    async def upsert_recipe(self, recipe_id: int, title: str, full_text: str, metadata: dict | None = None):
        if metadata is None: metadata = {"title": title}
    ...
    async def search(self, query: str, n_results: int = 5) -> list[int]:
        ...
        def _sync_search():
            return self.collection.query(
                query_embeddings=[query_vec_result], 
                n_results=n_results)
